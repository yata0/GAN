...
class generator(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(100,32*4*4*4)
        self.deconv1 = deconv(in_channels=32*4,out_channels=32*2,kernel_size=4,stride=2,padding=1,norm=True)
        self.deconv2 = deconv(in_channels=32*2,out_channels=32,kernel_size=4,stride=2,padding=1,norm=True)
        self.deconv3 = deconv(in_channels=32,out_channels=3,kernel_size=4,stride=2,padding=1,norm=False)
    def forward(self, x):
        out = self.fc1(x)
...
class discriminator(nn.Module):
    def __init__(self):
        ...
        self.output = nn.Linear(32*4*4*4,1)
